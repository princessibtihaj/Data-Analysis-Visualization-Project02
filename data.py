...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(filepath) as file:
            ...
            data_type = lines[1].replace(" ", "").split(',')

            if not set(data_type).intersection({'numeric', 'categorical'}):
                raise Exception("Unreadable Data")

            for i in range(len(header)):
                head = header[i].strip()
                data = data_type[i]
                if data in {'categorical', 'numeric'}:
                    self.headers.append(head)
                    self.header2col[head] = len(self.headers) - 1
                    if data == 'categorical':
                        self.cats2levels[head] = []
            ...
            for line in lines[2:]:
                ...
                for i, value in enumerate(values):
                    ...
                    if data_type[i] == 'categorical':
                        if not value:
                            value = 'Missing'
                        if value not in self.cats2levels[header[i]]:
                            self.cats2levels[header[i]].append(value)
                        parsed_row.append(self.cats2levels[header[i]].index(value))
                    elif data_type[i] == 'numeric':
                        parsed_row.append(np.nan if not value else float(value))
    ...
    def __str__(self):
        ...
        formatted_str = "-" * 10 + "\n" + f"{self.filepath} ({len(self.data)}x{len(self.headers)})\n"
        formatted_str += "Headers:\n" + " ".join(self.headers) + "\n" + "\n" * 10

        num_rows = len(self.data)
        display_rows = min(5, num_rows)
        formatted_str += f"{display_rows}/{num_rows} rows\n"
        
        for row in self.data[:display_rows]:
            formatted_str += " ".join(map(str, row)) + "\n"
    ...
    def get_num_dims(self):
        ...
        '''
        return len(self.header2col.keys())
    ...
    def select_data(self, headers, rows=[]):
        ...
        '''
        selected_columns = [self.data[:, self.header2col[header]] for header in headers]
        selected_data = np.column_stack(selected_columns) if not rows else np.column_stack(selected_columns)[rows]
