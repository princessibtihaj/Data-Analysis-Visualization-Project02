...
class CatData(data.Data):
    ...
    def get_cat_levels_int(self, header):
        ...
        '''
        return np.arange(0, len(self.cats2levels[header]))
    ...
    def filter(self, header, strlevel):
        ...
        '''
        required = self.str2intlevels(header, [strlevel])
        column = np.squeeze(self.select_data([header]))
        self.data = self.data[column == required]